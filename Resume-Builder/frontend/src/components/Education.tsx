...
const Education = () => {
  ...
  const onSubmit = async (values: any, onSubmitProps: any) => {
    try {
      if (isEdit) {
        await axios.put('/api/education', { education: values.education });
        toast.success(' Details Updated Successfully');
      } else {
        await axios.post('/api/education', { education: values.education });
  ...
  const handleDelete = async (formik: any, index: any) => {
    try {
      ...
      await axios.delete(`/api/education/${educationToDelete._id}`); //yha pe ek bbackslash hai dhyna dep apadega

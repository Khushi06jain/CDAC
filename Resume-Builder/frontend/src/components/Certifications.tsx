...
const Certifications = () => {
  ...
  useEffect(() => {
    if (subpages === "certifications") {
      const fetchData = async () => {
        try {
          const { data } = await axios.get("/api/certification");
          if (data) {
  ...
  const onSubmit = async (values: any, onSubmitProps: any) => {
    try {
      if (isEdit) {
        await axios.put("/api/certifications", values);
        toast.success(" Details Updated Successfully");
      } else {
        await axios.post("/api/certifications", values);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete("/api/certification");
